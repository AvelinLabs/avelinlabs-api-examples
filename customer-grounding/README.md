...
| `POST /api/v1/grounding/role-intelligence/reports` (Decision Passport opt-in) | Adds Decision Passport details as an opt-in body field. | `include_passport` is an optional boolean in the request body, default `false`; omitted or `false` returns the standard response shape. |
...
Use the same payload shape for all calls. Add `include_passport: true` only when Passport fields are needed.
...
passport_payload='customer-grounding/requests/role-intelligence-report-passport-include.example.json'

# include_passport omitted or false => default contract, no `passport` field in response
...
# include_passport=true => Decision Passport included as top-level `passport` (Level 1 example below)
curl -sS -X POST "${BASE_URL}/api/v1/grounding/role-intelligence/reports" \
  -H "Authorization: Bearer ${AVELIN_API_KEY}" \
  -H "Content-Type: application/json" \
  --data-binary "@${passport_payload}" \
  | python -m json.tool
```
...
In this repository the Passport is always returned in the same response envelope as a top-level `passport` field when requested; there is no separate Passport endpoint or query flag.
